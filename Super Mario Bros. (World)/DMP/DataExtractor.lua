...
if not csv then
    emu.log("ERROR abriendo:")
    emu.log(path)
    return
end

emu.log("Guardando en:")
emu.log(path)
-- Cabecera
csv:write(
    "A,B,UP,DOWN,LEFT,RIGHT,marioX,marioY,"
)
...
local function classifyTile(tile)
    ...
    if tile == 0x12 or tile == 0x13
       or tile == 0x14 or tile == 0x15 then
        return 3
    end
...
function Main()
    ...
    for row = 1,12 do
        ...
        for col = marioTile, marioTile + 6 do
            ...
            local tile =
                emu.read(addr, emu.memType.nesDebug)
	...
	for i = 0,4 do
	    ...
	    local x1 =
	        emu.read(base + 0, emu.memType.nesDebug)
	
	    local y1 =
	        emu.read(base + 1, emu.memType.nesDebug)
